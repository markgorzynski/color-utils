 ...
 * with Oklab L typically in the 0-1 range.
 */
...
// Assuming SrgbColor, LinearSrgbColor, OklabColor, and OklchColor are defined in types.js


// --- Constants for Oklab Transformations ---
// Based on Björn Ottosson's Oklab formulation.

// Step 1: Convert linear sRGB to "cone" responses (LMS)
// Note: These are not necessarily actual cone responses but intermediate values in the Oklab model.
...
// Step 2: Apply non-linearity (cube root) to LMS values (L_prime, M_prime, S_prime)
...
// Step 1: Convert Oklab (L,a,b) to non-linear LMS'
...
// Step 2: Apply inverse non-linearity (cubing) to LMS' values
...
export function linearSrgbToOklab(linearSrgbColor) {
  ...
  // Apply cube-root non-linearity. Handle negative results from matrix multiplication if they occur,
  // though for in-gamut sRGB, LMS values should be positive.
  // Oklab spec uses Math.cbrt which handles negative numbers correctly (maintains sign).
  const lmsPrime = [
    Math.cbrt(lms[0]),
    Math.cbrt(lms[1]),
    Math.cbrt(lms[2]),
 ...
 * Components may be outside [0, 1] if Oklab color is out of sRGB gamut.
 * @throws {TypeError} if oklabColor is not a valid OklabColor object.
 */
export function oklabToLinearSrgb(oklabColor) {
  if (typeof oklabColor !== 'object' || oklabColor === null ||
      typeof oklabColor.L !== 'number' || typeof oklabColor.a !== 'number' || typeof oklabColor.b !== 'number') {
    throw new TypeError('Input oklabColor must be an object with L, a, b number properties.');
  }
  const { L, a, b } = oklabColor;

  const lmsPrime = multiplyMatrixVector(MATRIX_OKLAB_TO_LMS_PRIME, [L, a, b]);

  // Apply cubing (inverse of cube-root non-linearity)
  const lms = [
...
export function oklchToOklab(oklchColor) {
  if (typeof oklchColor !== 'object' || oklchColor === null ||
      typeof oklchColor.L !== 'number' || typeof oklchColor.C !== 'number' || typeof oklchColor.h !== 'number') {
    throw new TypeError('Input oklchColor must be an object with L, C, h number properties.');
  }
...
/**
 * Converts an Oklab color object directly to an sRGB color object.
 * @param {OklabColor} oklabColor - The Oklab color object {L, a, b}.
 * @returns {SrgbColor} The sRGB color object {r, g, b}. (Components clamped by linearSrgbToSrgb if out of gamut)
 */
...
export function srgbToOklch(srgbColor) {
    const oklab = srgbToOklab(srgbColor);
    return oklabToOklch(oklab);
}

/**
 * Converts an OkLCh color object directly to an sRGB color object.
 * @param {OklchColor} oklchColor - The OkLCh color object {L, C, h}.
 * @returns {SrgbColor} The sRGB color object {r, g, b}.
 */
export function oklchToSrgb(oklchColor) {
    const oklab = oklchToOklab(oklchColor);
